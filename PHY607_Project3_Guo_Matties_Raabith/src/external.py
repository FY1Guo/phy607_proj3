...
def run_emcee(N_walkers, N_grid, J, h, beta=1.0, seed=123, steps=20000):
    ...
    N_spins = N_grid**2
    initial_state = rng.uniform(low=-0.01, high=0.01, size=(N_walkers, N_spins))
    ...
    sampler = emcee.EnsembleSampler(
        ...
        args=[J, h],
        kwargs={"beta": beta},
        moves=emcee.moves.MHMove(MHMove_proposal),
    )
...
if __name__ == "__main__":
    ...
    chain_steps = 100000
    mcmc = run_emcee(
        N_walkers, N_grid, 1, 0.1, seed=None, steps=burn_in_steps + chain_steps
    )
